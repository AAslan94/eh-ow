...
from designs_opt import designs
...
#connect
p_all =  np.sum(np.sum(l_amb.Pin_sm_diff, axis=0), axis=1) + np.sum(l_amb.Pin_sm, axis=0) + l_amb.Pin_sa + 0.5*x.Pin_sm_tot.flatten() #LOS + Diffuse + Ambient
...
G_ac = np.sum(np.sum(x.Pin_sm_diff, axis=0), axis=1)/x.A_sensor
G_ac = G_ac.flatten()
...
freq = np.linspace(100, 70000, 400)
...
for i in range(p_all.shape[0]):
    ...
    panels[i].set_circuit(Rc = 10, Lo=100e-6, Co=1000e-6)
    panels[i].find_bw()    
    ...
    thermal[i] = panels[i].th_noise[int(ind[i])]
...
print("The energy harvested is: " + str(e_day))
print("The energy consumed per day is: " + str(e_day_c))
print("The net energy is: " + str(e_day - e_day_c))
