...
params_amb['m_master'] = params_d['m_lights']
...
plt.ylabel('$W$ [m]')
plt.tight_layout()
#plt.savefig("e_cons_Nsun_FALSE.pdf", format="pdf", bbox_inches="tight", dpi=300)
plt.show()
...
print("For scenario B1 bandwidth varies from " + str(np.min(bw)) + " to " + str(np.max(bw)) + " Hz")
print("For scenario B1 irradiance varies from " + str(np.min(G_all)) + " to " + str(np.max(G_all)) + " W/m2")