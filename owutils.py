...
def lambertian_gains(rS, nS, rR, nR, mS, AR, FOV, SOLAR,calc_delays = False):
    ...
    a = np.ones(cos_theta.shape)
    #print("cos_theta is " + str(cos_theta))
    #print(rect_theta.shape)
    #print("+++")
    #print(rect_theta)
    #print("---")
    if SOLAR:
        ...
        #print('aperture is ' + str(a))
        #print(a[i] * cos_theta[i])
        #print('AAAA')
        #print('***')
        #print("--------------------------------------------")

    
    h[i] = (mS2[i] + 1) / (2 * np.pi * RR[i] ** 2.0 ) * (cos_phi[i] ** mS2[i]) * cos_theta[i] * AR2[i] * rect_theta[i] * a[i]
