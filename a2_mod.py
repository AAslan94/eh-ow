...
class RobustSensorEvaluator:
    ...
    def evaluate_batch(self, x_norm_batch):
        ...
        area  = 0.0001 + x_norm_batch[:, 2] * (0.02 - 0.0001)
...
def run_sensor_optimization(args):
    ...
    while not es.stop():
        ...
        if gen % log_every == 0:
            ...
            print(
                f"  [Sensor {idx+1:02d}] Gen {gen:4d}/{120} | "
    ...
    area  = 0.0001 + best[2] * (0.02 - 0.0001)
...
if __name__ == '__main__':
    ...
    nS_sen_list = np.round(
        align_receiver_to_transmitter(r_sen_list, np.array([5, 5, 3])), 2
    )
    N_sensors = r_sen_list.shape[0]

    base_params_d   = designs[KEY].copy()
    base_params_amb = designs['A2'].copy()
